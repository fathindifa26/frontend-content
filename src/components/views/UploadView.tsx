...
export function UploadView({ onUpload, onUrlSubmit, onRegenerate, isAnalyzing, data }: UploadViewProps) {
  ...
  // When new data arrives (initial analysis), set it as v1
  useEffect(() => {
    if (data?.ai_summary) {
      setVersionedSummaries({
        1: data.ai_summary
      });
      setActiveVersion(1);
    }
  ...
  const handleRegenerateClick = async (targetVersion?: number) => {
    ...
    if (newSummary) {
      setVersionedSummaries(prev => ({
        ...prev,
        [versionToUpdate]: newSummary
      }));
    }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="w-full">
        <div className={`glass-panel p-6 rounded-[32px] space-y-4 transition-all duration-500 ease-in-out ${benchmarkType === "frequency"
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="p-1 bg-white/5 rounded-xl border border-white/10 flex items-center relative overflow-hidden">
              <div
                className={`absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-lg transition-all duration-500 ease-out z-0 ${benchmarkType === "frequency"
              ...
              <button
                ...
                className={`relative z-10 px-4 py-1.5 text-[10px] font-bold transition-all duration-500 ${benchmarkType === "frequency" ? "text-white" : "text-white/30 hover:text-white/50"
                  }`}
              >
                Freq
              </button>
              <button
                ...
                className={`relative z-10 px-4 py-1.5 text-[10px] font-bold transition-all duration-500 ${benchmarkType === "views" ? "text-amber-400" : "text-white/30 hover:text-white/50"
                  }`}
              >
                Views
              </button>
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center space-x-4">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider"></h3>
            <div className="p-1 bg-white/5 rounded-xl border border-white/10 flex items-center relative overflow-hidden">
              <div
              ...
              {[1, 2, 3].map((v) => (
                <button
                  ...
                  className={`relative z-10 px-4 py-1.5 text-[10px] font-bold transition-all duration-500 w-12 ${activeVersion === v ? "text-white" : "text-white/30 hover:text-white/50"
                    }`}
                >
                  V{v}
                </button>
